...
%             step_first=floor(step_data_x(1,1))+start_number;                           %截取后的图像新坐标从0开始，而要读取的数据沿用了老的序列号，需要加上start_number才能读取正确信号。
%             step_end=floor(step_data_x(2,1))+start_number;
...
%             end
            ...
            data_d=sigDEN(data);                                                       %指令化的3级小波滤波，须在建立文件夹后第一时间将sigDEN函数文件拷到该文件夹。,五级滤掉太多了，统计图很难看。
            data_y=DNA_y_position_modi(step_first:step_end)';                           %得到y方向的波动数据，这一数据不能滤波
            data_x=DNA_x_position_modi(step_first:step_end)';
            % data_cal=data+(DNA_length-mean(data_d));
            
            %计算力值
            data_cal_mean=mean(data_d);           %Z方向均值，即L

            %         deviation_y=var(data_y);
            %         %y方向标准差^2=方差,改成了方差函数,已整合到force_estimate中
            %             force=force_estimate(data_y,data_cal_mean); 
            force=Kb_multi_T*data_cal_mean*1.0e-3/var_correction(data_y);  %力值计算，其中对方差进行了积分时间修正                               %除以1000，单位换算，最后单位是纳米，F单位是pN,为什么+1.4(1.4是2.8微米的球的半径，加上之后才是摆长)
            force_curve(force_number,1)=force_number;                      %force_curve第1列标明序号
            force_curve(force_number,2)=data_cal_mean;                     %force_curve 第2列输入Z值，也就是extension
            force_curve(force_number,3)=force;                             %第3列输入计算出的力值，即force
            force_curve(force_number,4)=magnet_z_position(step_first);     %第4列输入Zmag
            
            % data_input(:,2)=data_4_analysis;                                           %输入数据（是一种很有参考价值的写法）第一列为序号，第二列为数据
            % data_input(:,1)=(1:size(data_4_analysis,1))';
            title_name=strcat('F=',num2str(force),'length=',num2str(data_cal_mean),'Z=',num2str(magnet_z_position(step_first)),'NO_',num2str(deal_number));  %增加序号和磁镊位置，方便对应查找。
    ...
    figure;
    plot(force_curve(:,2)',force_curve(:,3)','*');
