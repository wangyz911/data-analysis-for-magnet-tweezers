% 本脚本全称： force distribution --> rate --> landscape
% 是基于文献Proc. Natl. Acad. Sci. U.S.A. 110,
% 16432–7.中的工作编写的，用于处理含有多个态的结构在展开时如何得到能量图景。
%需要已知各个态分别的力值分布，往复或重叠的情况照常记录下力值,但正跳和回跳有所区别，统计时需要区别对待。
%输入应是每个态一个 矩阵NI,IN,IU,UI，第一行是bin值(F)，第二行是counts of F.
%matlab 的拟合不太给力，还是要用origin
...
%修改了hist 的生成方法，采用freedman法则，使其更适用于拖尾的数据
figure;
IU_h = histogram(IU_f,'BinMethod','fd');
%力值分布直方图
bin_number_IU =IU_h.NumBins;
bin_width_IU = IU_h.BinWidth;%待定
force_IU =IU_h.BinEdges';                                                         %这里力值序列应该等于贯穿整个force
...
% 
% NI_rate = zeros(bin_number,1);                                    %这里要解释一下，暂时默认为渐进模型，1-2-3，因此三个状态总共有两个转换速率，
